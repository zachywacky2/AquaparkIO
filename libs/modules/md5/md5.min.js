var md5 = function() {
    ...
    var r = (__define, t);
    return p = r.prototype, p.hex_md5 = function(t) {
        ...
        for (var h = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", i = "", s = t.length, d = 0; s > d; d += 3)
            for (var n = t.charCodeAt(d) << 16 | (s > d + 1 ? t.charCodeAt(d + 1) << 8 : 0) | (s > d + 2 ? t.charCodeAt(d + 2) : 0), _ = 0; 4 > _; _++) i += 8 * d + 6 * _ > 8 * t.length ? this.b64pad : h.charAt(n >>> 6 * (3 - _) & 63);
        return i
    }, p.rstr2any = function(t, r) {
        var h, i, s, d, n, _ = r.length,
            m = Array(Math.ceil(t.length / 2));
        for (h = 0; h < m.length; h++) m[h] = t.charCodeAt(2 * h) << 8 | t.charCodeAt(2 * h + 1);
        var e = Math.ceil(8 * t.length / (Math.log(r.length) / Math.log(2))),
            f = Array(e);
        for (i = 0; e > i; i++) {
